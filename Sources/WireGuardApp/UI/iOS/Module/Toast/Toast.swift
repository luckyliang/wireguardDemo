...
//  Games
//
//  Created by Django on 17/08/2019.
...
fileprivate extension UIView {
    ...
    private var _activeToasts: NSMutableArray {
        get {
    ...
    private var _queue: NSMutableArray {
        get {
     ...
     */
    func _makeToast(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, title: String? = nil, image: UIImage? = nil, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)? = nil) {
        do {
            let toast = try _toastViewForMessage(message, title: title, image: image, style: style)
            _showToast(toast, duration: duration, position: position, completion: completion)
        } catch ToastError.missingParameters {
            let errString = "Error: message, title, and image are all nil"
            printDebug(errString, assertMessage: errString)
        } catch {}
    }
    ...
    /**
     ...
     didTap will be `true` if the toast view was dismissed from a tap.
     */
    func _makeToast(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, point: CGPoint, title: String?, image: UIImage?, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)?) {
        do {
            let toast = try _toastViewForMessage(message, title: title, image: image, style: style)
            _showToast(toast, duration: duration, point: point, completion: completion)
        } catch ToastError.missingParameters {
            let errString = "Error: message, title, and image cannot all be nil"
            printDebug(errString, assertMessage: errString)
        } catch {}
    }
    ...
    /**
     ...
     didTap will be `true` if the toast view was dismissed from a tap.
     */
    func _showToast(_ toast: UIView, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, completion: ((_ didTap: Bool) -> Void)? = nil) {
        let point = position._centerPoint(forToast: toast, inSuperview: self)
        _showToast(toast, duration: duration, point: point, completion: completion)
    }
    ...
    /**
     ...
     didTap will be `true` if the toast view was dismissed from a tap.
     */
    func _showToast(_ toast: UIView, duration: TimeInterval = ToastManager.shared.duration, point: CGPoint, completion: ((_ didTap: Bool) -> Void)? = nil) {
        objc_setAssociatedObject(toast, &ToastKeys.completion, ToastCompletionWrapper(completion), .OBJC_ASSOCIATION_RETAIN_NONATOMIC);

        if ToastManager.shared.isQueueEnabled, _activeToasts.count > 0 {
            ...
            _showToast(toast, duration: duration, point: point)
        }
    }
     ...
     */
    func _hideToast() {
        guard let activeToast = _activeToasts.firstObject as? UIView else { return }
     ...
     */
    func _hideToast(_ toast: UIView) {
        guard _activeToasts.contains(toast) else { return }
        _hideToast(toast, fromTap: false)
    }
     ...
     */
    func _hideAllToasts(includeActivity: Bool = false, clearQueue: Bool = true) {
        if clearQueue {
            _clearToastQueue()
        }
    ...
    /**
     ...
     @param position The toast's position
     */
    func _makeToastActivity(_ position: ToastPosition = .center) {
        // sanity
        guard objc_getAssociatedObject(self, &ToastKeys.activityView) as? UIView == nil else { return }

        let toast = _createToastActivityView(position: position)
        let point = position._centerPoint(forToast: toast, inSuperview: self)
        _makeToastActivity(toast, point: point)
    ...
    /**
     ...
     @param point The toast's center point
     */
    func _makeToastActivity(_ point: CGPoint) {
        // sanity
        guard objc_getAssociatedObject(self, &ToastKeys.activityView) as? UIView == nil else { return }

        let toast = _createToastActivityView(point: point)
        _makeToastActivity(toast, point: point)
    }
    ...
    private func _makeToastActivity(_ toast: UIView, point: CGPoint) {
        toast.alpha = 0.0
        toast.center = point
    ...
    private func _createToastActivityView(position: ToastPosition? = nil,
                                          point: CGPoint? = nil) -> UIView {
        ...
        animationView.frame = activityView.bounds
    ...
    private func _showToast(_ toast: UIView, duration: TimeInterval, point: CGPoint) {
        toast.center = point
        toast.alpha = 0.0

        if ToastManager.shared.isTapToDismissEnabled {
            let recognizer = UITapGestureRecognizer(target: self, action: #selector(UIView._handleToastTapped(_:)))
        ...
        UIView.animate(withDuration: ToastManager.shared.style.fadeDuration, delay: 0.0, options: [.curveEaseOut, .allowUserInteraction], animations: {
            ...
            let timer = Timer(timeInterval: duration, target: self, selector: #selector(UIView._toastTimerDidFinish(_:)), userInfo: toast, repeats: false)
            RunLoop.main.add(timer, forMode: .common)
            objc_setAssociatedObject(toast, &ToastKeys.timer, timer, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }

    private func _hideToast(_ toast: UIView, fromTap: Bool) {
        ...
        UIView.animate(withDuration: ToastManager.shared.style.fadeDuration, delay: 0.0, options: [.curveEaseIn, .beginFromCurrentState], animations: {
            ...
            toast.removeFromSuperview()
            self._activeToasts.remove(toast)
            ...
            if let nextToast = self._queue.firstObject as? UIView, let duration = objc_getAssociatedObject(nextToast, &ToastKeys.duration) as? NSNumber, let point = objc_getAssociatedObject(nextToast, &ToastKeys.point) as? NSValue {
                self._queue.removeObject(at: 0)
                self._showToast(nextToast, duration: duration.doubleValue, point: point.cgPointValue)
            }
        }
    }
    ...
    @objc
    private func _handleToastTapped(_ recognizer: UITapGestureRecognizer) {
        guard let toast = recognizer.view else { return }
        _hideToast(toast, fromTap: true)
    }

    @objc
    private func _toastTimerDidFinish(_ timer: Timer) {
     ...
     */
    func _toastViewForMessage(_ message: String?, title: String?, image: UIImage?, style: ToastStyle) throws -> UIView {
        // sanity
...
public enum ToastPosition {
    ...
    fileprivate func _centerPoint(forToast toast: UIView, inSuperview superview: UIView) -> CGPoint {
...
extension UIView {
    ...
    /// 弹出 toast
    /// - Parameter message: 具体提示内容
    /// - Parameter duration: 持续时间 默认为 ToastManager.shared.duration
    /// - Parameter position: 位置 默认为 ToastManager.shared.position
    /// - Parameter title: 提示的 title 默认为空
    /// - Parameter image: 提示的图片 默认为空
    /// - Parameter style: 样式 默认为 ToastManager.shared.style
    /// - Parameter completion: 结束 Block 逃逸闭包
    public func makeToast(_ message: String,
        ...
        let durationT = duration ?? ToastManager.shared.duration
        let positionT = position ?? ToastManager.shared.position
        let styleT = style ?? ToastManager.shared.style
    ...
    /// 弹出菊花
    /// - Parameter position: 位置 默认 ToastPosition.center
    public func makeActivity(_ position: ToastPosition = .center) {
        _makeToastActivity(position)
    }

    /// 弹出菊花
    /// - Parameter point: 位置 具体 Point
    public func makeActivity(_ point: CGPoint) {
        _makeToastActivity(point)
    }

    /// 隐藏 toast
    /// - Parameter includeActivity: 是否一并隐藏菊花
    /// - Parameter clearQueue: 清除当前 view 下所有 toast
    public func hideToast(includeActivity: Bool = true,
                          clearQueue: Bool = true) {
        _hideAllToasts(includeActivity: includeActivity,
